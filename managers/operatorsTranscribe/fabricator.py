...
class Fabricator(object):
    ...
    **Available Methods:**
    ...
    +----------------------------------------------------+---------------------+
    | :py:meth:`.construct_nwbseries_regionbodies`       | class method        |
    +----------------------------------------------------+---------------------+
    | :py:meth:`.construct_nwbseries_components`         | class method        |
    +----------------------------------------------------+---------------------+
    | :py:meth:`.generic_timeseries`                     | static method       |
    +----------------------------------------------------+---------------------+
    | :py:meth:`.link_nwbseries_regionbodies_to_nwbfile` | static method       |
    +----------------------------------------------------+---------------------+
    | :py:meth:`.link_nwbseries_components_to_nwbfile`   | static method       |
    +----------------------------------------------------+---------------------+
    | :py:meth:`.nwbepochs_regionbodies`                 | static method       |
    +----------------------------------------------------+---------------------+
    | :py:meth:`.nwbepochs_components`                   | static method       |
    +----------------------------------------------------+---------------------+
    ...
    @staticmethod
    def nwbepochs_regionbodies( model, epochmd, nwbseries, nwbfile ):
        regionlist = rp.get_regionlist(model)
        for a_region_name in regionlist:
            no_of_rec = len(model.regions[a_region_name])
            for ith_rec_type in range(no_of_rec):
                rec_of = model.regions[a_region_name][ith_rec_type]
                no_of_epochs = epochmd["epoch0"+a_region_name][rec_of]["tags"][0][0]
                for epochID in range(int(no_of_epochs)):
                    md_of_interest = epochmd["epoch"+str(epochID)+a_region_name][rec_of]
                    nwbfile.add_epoch(
                           start_time = float( md_of_interest["start_time"] ),
                           stop_time = float( md_of_interest["stop_time"] ),
                           tags = md_of_interest["tags"],
                           timeseries = nwbseries[a_region_name][rec_of] )
        return nwbfile

    @staticmethod
    def nwbepochs_components( model, epochmd, nwbseries, nwbfile ):
        componentgrouplist = rp.get_componentgrouplist(model)
        for compgroup_name in componentgrouplist:
            its_regionlist = rp.get_regionlist_of_componentgroup(model, compgroup_name)
            for a_region_name in its_regionlist:
                ...
                for a_comp_name in complist:
                    ...
                    for ith_rec_type in range(no_of_rec):
                        rec_of = model.regions[compgroup_name][a_region_name]\
                                              [a_comp_name][ith_rec_type]
                        no_of_epochs = \
                           epochmd["epoch0"+compgroup_name][a_region_name]\
                                  [a_comp_name][rec_of]["tags"][0][0]
                        for epochID in range(int(no_of_epochs)):
                             md_of_interest = \
                                epochmd["epoch"+str(epochID)+compgroup_name]\
                                       [a_region_name][a_comp_name][rec_of]
                             nwbfile.add_epoch(
                                start_time = float( md_of_interest["start_time"] ),
                                stop_time = float( md_of_interest["stop_time"] ),
                                tags = md_of_interest["tags"],
                                timeseries = nwbseries[compgroup_name][a_region_name]
                                                      [a_comp_name][rec_of] )
        return nwbfile

    @classmethod
    def build_nwbepochs( cls, chosenmodel=None, epochmd=None, nwbfile=None, nwbts=None ):
        ...
        *NOTE:* If we are interested in extracting an epoch in a more organized manner for instance epoch of interest may be a particular epoch for recording done from a particular region, then using the above method of extraction based on picking a row of the `VectorTable <https://pynwb.readthedocs.io/en/latest/pynwb.core.html#pynwb.core.VectorData>`_ is not sufficient. The more improved approach of extraction will be done by the manager.
        ...
        """
        nwbfile = cls.nwbepochs_regionbodies( chosenmodel, epochmd, nwbts, nwbfile )
        componentgrouplist = rp.get_componentgrouplist(chosenmodel)
        if len(componentgrouplist) != 0:
            nwbfile = cls.nwbepochs_components( chosenmodel, epochmd, nwbts, nwbfile)
        return nwbfile
...
# DEPRECIATED FROM BELOW
    @classmethod
    def def_build_nwbepochs( cls, epochmd=None, nwbfile=None, nwbts=None ):
        """method for contructing epochs into the built nwbfile
    ...
    @classmethod
    def dep_build_nwbepochs( cls, epochmd=None, nwbfile=None, nwbts=None ):
        """method for contructing epochs into the built nwbfile
    ...
    @classmethod
    def dep_write_nwbfile(cls, nwbfile=None, filepath=None):
        """Writes the created NWB file into a HDF5 file in the given filepath.
