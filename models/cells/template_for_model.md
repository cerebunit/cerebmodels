...
**For uploading models in `~/cerebmodels/models/cells/`, i.e., u follow the manadatory steps instructed below.**
...
#### 2.1.2 Make available the modules within `CerebModels` that would be need to execute the simulation
...
#### 2.1.3 Import `SciUnit` module and the particular __capability__ the model will have
```
import sciunit
...
```
This is a mandatory step if one intends to validate the model via the HBP Validation Framework. After the model is registered to the HBP Validation Framework Model catalog, it assigns a uuid. This generated uuid will be the value for above.
...
Once the method name is consistent with the inherited `<BrandCategory>` capability one may write it however one wants, tailored to the validation test in `CerebUnit` that would call it.

However, below is a template for how might go about writing it.

#### 2.4.1 Simplest case scenario
```
sm.engage_NEURON()
```

#### 2.4.2 Setting up `ExecutiveControl`
```
ec = ExecutiveControl() # only works when in ~/cerebmodels
model = 
```

#### 2.4.3 Launching the model, i.e., simulate
```
ec.launch_model( parameters = runtime_parameters,         # dictionary
                 stimparameters = stimulation_parameters, # dictionary
                 stimloc = model.cell.soma,
                 onmodel = model,
                 capabilities = {"model": "a_method_name_this_model",
                                 "vtest": <BrandCategory>} )
ec.save_response()
```
