#Template of the Granule cell model (multi-compartment), Dover et al. 2016
...
class Granule(object):
    ...
    def __init__(self):
        ...
        self.pf = [h.Section(name="pf"+str(i)) for i in range(npf)]
        self.br0 = h.Section(name="br0")
        self.br1 = h.Section(name="br1")
        self.br2 = h.Section(name="br2")
        self.br3 = h.Section(name="br3")
        ...
        for w3 in range(naxon):
                self.axon[w3].nseg = 1
                self.axon[w3].diam = 0.15
                self.axon[w3].L = 2.3367
                self.axon[w3].cm = 1
                self.axon[w3].Ra = 100

                self.axon[w3].insert("GRC_LKG3")
        ...
        # Definitions for passive compartments (maintains propagabtion delay)
        self.branch0.nseg = 1
        self.branch0.diam = 0.15
        self.branch0.L = 3
        self.branch0.cm = 1
        self.branch0.Ra = 100
        #self.branch0.celsius = 30 # celsius not section variable instead its h.celsius
        self.branch0.insert("GRC_LKG3")

        self.branch1.nseg = 1
        self.branch1.diam = 0.1
        self.branch1.L = 5
        self.branch1.cm = 1
        self.branch1.Ra = 100
        #self.branch1.celsius = 30 # celsius not section variable instead its h.celsius
        self.branch1.insert("GRC_LKG3")

        self.branch2.nseg = 1
        self.branch2.diam = 0.05
        self.branch2.L = 10
        self.branch2.cm = 1
        self.branch2.Ra = 100
        #self.branch3.celsius = 30 # celsius not section variable instead its h.celsius
        self.branch2.insert("GRC_LKG3")

        self.branch3.nseg = 1
        self.branch3.diam = 0.025
        self.branch3.L = 20
        self.branch3.cm = 1
        self.branch3.Ra = 100
        #self.branch3.celsius = 30 # celsius not section variable instead its h.celsius
        self.branch3.insert("GRC_LKG3")

        # Pf definition
        for p1 in range(npf):
                self.pf[p1].nseg = 1
                self.pf[p1].diam = 0.15
                self.pf[p1].L = 10
                self.pf[p1].cm = 1
                self.pf[p1].Ra = 100

                self.pf[p1].insert("GRC_LKG3")
                self.pf[p1].insert("GRC_NA")
                self.pf[p1].insert("GRC_KV")

                h.usetable_GRC_KV = 0

                self.pf[p1].ena = 87.39
                self.pf[p1].ek = -84.69

        # Definitions for passive compartments
        self.br0.nseg = 1
        self.br0.diam = 0.15
        self.br0.L = 3
        self.br0.cm = 1
        self.br0.Ra = 100
        #self.br0.celsius = 30 # celsius not section variable instead its h.celsius
        self.br0.insert("GRC_LKG3")
        ...
        self.axon[naxon-1].connect( self.pf[250], 1, 0 ) # 0 or 1 not 0.5 thus used 0

        [ self.pf[i].connect( self.pf[i+1], 1, 0 ) for i in range(npf-1) ]

        #self.branch0.connect( self.pf[0], 0, 0 )
        #self.branch0.connect( self.branch1, 1, 0 )
        #self.branch1.connect( self.branch2, 1, 0 )
        #self.branch2.connect( self.branch3, 1, 0 )

        #self.pf[npf-1].connect( self.br0, 1, 0 )
        #self.br0.connect( self.br1, 1, 0 )
        #self.br1.connect( self.br2, 1, 0 )
        #self.br2.connect( self.br3, 1, 0 )

        #area_of_soma = self.soma(0.5).area() * 1e-08
        #area_of_soma = math.pi * self.soma.diam * self.soma.L * 1e-08 # alternatively
